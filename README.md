...
This project extends the original Desktop Pet foundation with a pet-first startup flow,
smoother cached animation rendering, corrected spritesheet handling, and documented
replaceable character packs. Personal Naruto artwork is intentionally kept local and is
...
The companion settings window can be opened from the menu. `Persona` controls the character name, personality, speaking style, background, and extra rules. If left empty, the app uses the current character pack's default style.
