...
function draw() {
  ...
  const key = `${state}:${frameIndex % current.frames}`;
  let frameCanvas = frameCache.get(key);
  if (!frameCanvas) {
    ...
    frameContext.drawImage(
      spriteImage,
      (frameIndex % current.frames) * frame.width,
      current.row * frame.height,
      frame.width,
      frame.height,
      0,
      0,
    ...
    frameCache.set(key, frameCanvas);
  }
...
function startAnimation() {
  ...
  const tick = (now) => {
    ...
    const current = character.states[animationState] || character.states.idle;
    const interval = Math.max(16, Number(current.interval) || 100);
    const elapsed = now - lastFrameTime;
    if (elapsed >= interval) {
      const steps = Math.floor(elapsed / interval);
      frameIndex = (frameIndex + steps) % current.frames;
...
function scheduleIdleBehavior() {
  ...
  idleTimer = window.setTimeout(() => {
    if (state === "idle") {
      ...
      playTemporary(choices[Math.floor(Math.random() * choices.length)]);
    }
...
function temporaryDuration(nextState) {
  ...
  if (nextState === "sleeping") return 3500;
